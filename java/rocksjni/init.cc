...
namespace rocksdb {
	...
	void setLoader(callback func){
		...
		std::cout << "add a new loader. nb loaders in loaders: " << loaders.size() << std::endl;
	}
	void setUnloader(callback func){
		rocksdb::unloaders.push_back(func);
		std::cout << "add a new unloader. nb unloaders: " << unloaders.size() << std::endl;
	}
    ...
    if (context == NULL) {
      ...
      if (getEnvStat == JNI_EDETACHED) {
        ...
        if (rocksdb::JVM->AttachCurrentThread((void **) &env, NULL) != JNI_OK) {
            ...
            if (jc)
							env->ThrowNew(jc, "unable to attach JNIEnv");
            return NULL;
        } else {
        ...
        jclass jc = env->FindClass("java/lang/Error");
        if (jc)
					env->ThrowNew(jc, "unable to create JNIEnv:version not supported");
        return NULL;
...
JNIEXPORT void JNICALL Java_org_rocksdb_util_Environment_detachCurrentThreadIfPossible(JNIEnv* /*env*/, jclass /*clazz*/) {
  rocksdb::detachCurrentThread();
}
...
jint JNI_OnLoad(JavaVM* vm, void* /*reserved*/) {
  ...
  JNIEnv *env = rocksdb::getEnv();
	...
	std::cout << "########### nb loaders: " << rocksdb::loaders.size() << std::endl;
